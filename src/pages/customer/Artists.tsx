  ...
  return (
    <Template>
      <div className='flex-1 bg-gradient-to-b from-primary to-black p-8 overflow-auto pb-28'>
        <h2 className='text-2xl font-bold text-primaryText mb-6'>Popular Artists</h2>
        <div className='grid grid-cols-2 lg:grid-cols-3 gap-6'>
          {artists.map((artist) => (
            <div key={artist.id} className='flex flex-col items-center'>
              ...
              />
              <h3 className='text-primaryText font-semibold mt-2'>{artist.name}</h3>
              <p className='text-gray-400 text-sm'>{formatListenCount(artist.listenCount)}</p>
            </div>
          ))}
        </div>
      </div>
